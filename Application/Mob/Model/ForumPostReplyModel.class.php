...
class ForumPostReplyModel extends Model
{
    ...
    public function delPostReply($id)
    {
        ...
        $reply_list=D('ForumPostReply')->where(array('post_id'=>$reply['post_id']))->field('id')->select();
        $reply_count=count($reply_list);
        $reply_list=array_column($reply_list,'id');
        $reply_count+=D('ForumLzlReply')->where(array('id'=>array('in',$reply_list)))->count();
        D('ForumPost')->where(array('id' => $reply['post_id']))->setField('reply_count',$reply_count);
